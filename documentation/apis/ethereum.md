...
{% hint style="warning" %}
This method only works for filters creates with [`eth_newFilter`](ethereum.md#eth_newfilter)not for filters created using [`eth_newBlockFilter`](ethereum.md#eth_newblockfilter) or [`eth_newPendingTransactionFilter`](ethereum.md#eth_newpendingtransactionfilter), which will return `"filter not found".`
...
Creates a filter in the node, to notify when a new block arrives. To check if the state has changed, call [`eth_getFilterChanges`](ethereum.md#eth_getfilterchanges).
...
To check if the state has changed, call [`eth_getFilterChanges.`](ethereum.md#eth_getfilterchanges)\`\`
...
Creates a filter in the node, to notify when new pending transactions arrive.To check if the state has changed, call [`eth_getFilterChanges`](ethereum.md#eth_getfilterchanges)\`\`
...
Uninstalls a filter with given id. Should always be called when watch is no longer needed. Additionally, Filters timeout when they aren’t requested with [`eth_getFilterChanges`](ethereum.md#eth_getfilterchanges)for a period of time.
